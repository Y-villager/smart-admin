    ...
    <a-form class="smart-query-form">
        <a-row class="smart-query-form-row">
            <a-form-item label="单据日期" class="smart-query-form-item">
                <a-date-picker valueFormat="YYYY-MM-DD" v-model:value="queryForm.billDate" style="width: 200px" />
            </a-form-item>
            <a-form-item label="单据日期" class="smart-query-form-item">
                <a-input style="width: 200px" v-model:value="queryForm.billNo" placeholder="单据日期" />
            </a-form-item>
          ...
          <a-form-item label="源单编号" class="smart-query-form-item">
            ...
            <a-form-item class="smart-query-form-item">
                ...
                <a-button @click="resetQuery" class="smart-margin-left10" v-privilege="'fSales:query'">
                    <template #icon>
                        <ReloadOutlined />
                    </template>
                    重置
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
        ...
        <a-row class="smart-table-btn-block">
            <div class="smart-table-operate-block">
                <a-button @click="showForm" type="primary" v-privilege="'fSales:add'">
                    <template #icon>
                        <PlusOutlined />
                    </template>
                    新建
                ...
                <a-button @click="confirmBatchDelete" type="primary" danger :disabled="selectedRowKeyList.length === 0"
                          v-privilege="'fSales:batchDelete'">
                    <template #icon>
                        <DeleteOutlined />
                    </template>
                    批量删除
                </a-button>
              <a-button @click="showImportModal" type="primary" v-privilege="'fSales:import'">
                <template #icon>
                  <ImportOutlined />
                </template>
                导入
              ...
              <a-button @click="onExportFSales" type="primary" v-privilege="'fSales:export'" :loading="exportLoading">
                <template #icon>
                  <ExportOutlined />
                </template>
                导出
              ...
              <!-- <template v-if="column.dataIndex === 'dict'">
                ...
                <template v-if="column.dataIndex === 'action'">
                    <div class="smart-table-operate">
                        <a-button @click="showForm(record)" type="link" v-privilege="'fSales:update'">编辑</a-button>
                        <a-button @click="onDelete(record)" danger type="link" v-privilege="'fSales:delete'">删除</a-button>
                    </div>
                </template>
            </template>
        </a-table>
        ...
        <FSalesForm  ref="formRef" @reloadList="queryData"/>
...
<script setup>
    ...
    import { fSalesApi } from '/@/api/vigorous/f-sales-api';
    import { PAGE_SIZE_OPTIONS } from '/@/constants/common-const';
    import { smartSentry } from '/@/lib/smart-sentry';
    import TableOperator from '/@/components/support/table-operator/index.vue';
    import {excelApi} from "/@/api/vigorous/excel-api.js";

    import FSalesForm from './f-sales-form.vue';
    ...
    async function queryData() {
        ...
        try {
            let queryResult = await fSalesApi.queryPage(queryForm);
    ...
    async function requestDelete(data){
        ...
        try {
            ...
            await fSalesApi.delete(data.fSalesId);
    ...
    async function requestBatchDelete() {
        try {
            SmartLoading.show();
            await fSalesApi.batchDelete(selectedRowKeyList.value);
            message.success('删除成功');
            queryData();
        } catch (e) {
            smartSentry.captureError(e);
...
async function onExportFSales() {
  exportLoading.value = true
  await fSalesApi.exportFSales(queryForm);
...
async function onImportFSales() {
  ...
  SmartLoading.show();
  try {
    let res = await fSalesApi.importFSales(formData);
